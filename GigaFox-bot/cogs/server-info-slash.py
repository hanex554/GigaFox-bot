...
class ServerInfo1(commands.Cog):
    ...
    async def server(self, ctx):
        ...
        if guild is not None:
            ...
            verification_level_str = verification_levels.get(guild.verification_level.value, "Неизвестно")
            ...
            # Проверка наличия значка и установка миниатюры
            if ctx.guild.icon:
                embed.set_thumbnail(url=ctx.guild.icon.url)

            # Добавление информации о сервере во вставку
            embed.add_field(name="**Владелец сервера**", value=ctx.guild.owner.name, inline=True)
            ...
            embed.add_field(name="**Каналы**",
                            ...
                            inline=True)
            embed.add_field(name="**Уровень проверки сервера**", value=verification_level_str, inline=True)
